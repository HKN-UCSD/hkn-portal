...
def rsvp(request, data):
    ...
    if (
        event_to_check_time_of.event_type
        and event_to_check_time_of.is_time_restricted
        and event_to_check_time_of.event_type.name == "Outreach"
        and timezone.now() + timedelta(days=1) > event_to_check_time_of.start_time
    ):
        raise ForbiddenException(
            "You can only RSVP more than 24 hours before an Outreach event"
        )
    try:
        acted_on = CustomUser.objects.get(user_id=data["acted_on"])
