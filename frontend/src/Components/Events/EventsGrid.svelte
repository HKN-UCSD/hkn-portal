    ...
    try {
      ...
      if (response.ok) {
        ...
        let userRecordResponse = await fetch(
          `/api/eventactionrecords/user/${userData.user_id}/`
        );
...
<div class=" mx-5 bg-gray-50 md:mx-auto text-primary">
  <div
    class="border border-gray-300 rounded-lg shadow-md p-6 hover:shadow-xl transform transition-transform duration-300 ease-in-out"
  >
    ...
    {#if events.length == 0}
      <div class="flex flex-col md:flex-col max-h-[1000px] overflow-x-auto">
        <div
          class="flex flex-col items-center justify-center w-full h-full py-10"
        >
