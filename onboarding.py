...
previous_onboarding, created = Onboarding.objects.get_or_create(quarter="Previous", new_officer=False)
...
officers = Officer.objects.select_related('user', 'user__induction_class')

for officer in officers:
    # Access induction class from Officer to CustomUser
    induction_class = officer.user.induction_class

    if induction_class == "Beta Omicron":
        officer.onboarding = spring_2024_onboarding
    else:
